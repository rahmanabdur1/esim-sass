...
const ok = <T>(data: T, meta?: { total?: number; page?: number; limit?: number }) => ({
  data: {
    success: true,
    data: meta
      ? { data, total: meta.total ?? 0, page: meta.page ?? 1, limit: meta.limit ?? 50 }
      : data,
    message: 'OK',
  },
  status: 200,
...
async function mockRoute(
  ...
  if (m === 'POST' && u === 'auth/forgot-password')
    return ok({ message: 'Reset link sent to your email.' });
  if (m === 'POST' && u === 'auth/reset-password')
    return ok({ message: 'Password reset successfully.' });
  if (m === 'POST' && u === 'auth/verify-email')
    return ok({ message: 'Email verified successfully.' });
  if (m === 'POST' && u === 'auth/resend-verification')
  ...
  if (m === 'GET' && u.startsWith('plans')) return ok(MOCK_PLANS, { total: MOCK_PLANS.length });

  // ── COUNTRIES ─────────────────────────────────────────────────────────────
  if (m === 'GET' && u === 'countries/featured') return ok(MOCK_COUNTRIES.slice(0, 8));
  ...
  if (m === 'GET' && u.startsWith('esims')) return ok(MOCK_ESIMS, { total: MOCK_ESIMS.length });
  ...
  if (m === 'GET' && u.startsWith('orders')) return ok(MOCK_ORDERS, { total: MOCK_ORDERS.length });
  ...
  // ── USER / PROFILE ────────────────────────────────────────────────────────
  if (m === 'GET' && (u === 'user/profile' || u === 'users/me')) return ok(MOCK_USER);
  ...
  if (m === 'GET' && u === 'users/me/export')
    return ok({
      downloadUrl: '#mock-export-download',
      expiresAt: new Date(Date.now() + 3600000).toISOString(),
    });
  ...
  if (m === 'DELETE' && u.match(/^users\/me\/sessions\//)) return ok({ success: true });
  ...
  if (m === 'PATCH' && u.match(/^user\/notifications\/[^/]+\/read$/)) return ok({ success: true });

  if (m === 'PATCH' && u === 'user/notifications/read-all') return ok({ success: true });

  if (m === 'GET' && u.startsWith('notifications'))
    return ok(MOCK_NOTIFICATIONS, { total: MOCK_NOTIFICATIONS.length });

  if (m === 'PATCH' && u.match(/^notifications\/[^/]+\/read$/)) return ok({ success: true });

  if (m === 'POST' && u === 'notifications/read-all') return ok({ success: true });
  ...
  // ── PAYMENT METHODS ───────────────────────────────────────────────────────
  if (m === 'GET' && u === 'user/payment-methods') return ok(MOCK_PAYMENT_METHODS);
  ...
  if (m === 'DELETE' && u.match(/^user\/payment-methods\/[^/]+$/)) return ok({ success: true });

  if (m === 'PATCH' && u.match(/^user\/payment-methods\/[^/]+\/default$/))
    return ok({ success: true });

  // Also handle the /payment-methods route (without /user/ prefix)
  if (m === 'GET' && u.startsWith('payment-methods')) return ok(MOCK_PAYMENT_METHODS);
  ...
  if (m === 'DELETE' && u.match(/^payment-methods\/[^/]+$/)) return ok({ success: true });

  if (m === 'PATCH' && u.match(/^payment-methods\/[^/]+\/default$/)) return ok({ success: true });
  ...
  // ── REFERRAL ──────────────────────────────────────────────────────────────
  if (m === 'GET' && (u === 'user/referral' || u.startsWith('referral'))) return ok(MOCK_REFERRAL);

  if (m === 'POST' && u === 'user/referral/send')
    return ok({ message: 'Referral invitation sent!' });

  // ── REWARDS ───────────────────────────────────────────────────────────────
  if (m === 'GET' && (u === 'user/rewards' || u.startsWith('rewards'))) return ok(MOCK_REWARDS);
  ...
  // ── SETTINGS ─────────────────────────────────────────────────────────────
  if (m === 'PATCH' && u === 'user/settings') return ok({ ...MOCK_USER, ...(body as object) });
