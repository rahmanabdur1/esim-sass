  ...
  try {
    ...
    if (USE_MOCK) {
      ...
      response.cookies.set({
        name: 'esim_access_token', value: `mock_token_${Date.now()}`,
    ...
    if (!res.ok) return NextResponse.json({ error: data.message ?? 'Invalid credentials' }, { status: res.status });

    const response = NextResponse.json({ user: data.user, message: 'Login successful' }, { status: 200 });
    response.cookies.set({ name: 'esim_access_token', value: data.token, httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/', maxAge: 60 * 60 * 24 * 30 });
    return response;
  } catch {
