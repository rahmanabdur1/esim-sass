...
export default function BuyPlanPage() {
  ...
  const { data: plansData, isLoading: plansLoading } = usePlans({ ...filters, search: search || undefined });
  const { data: countries, isLoading: countriesLoading } = useCountries();

  const handleSelect = useCallback((plan: Plan) => {
    setItem({ planId: plan.id, planName: plan.name, price: plan.price, currency: plan.currency, data: plan.data, validity: plan.validity, countryName: plan.country.name });
    addRecentlyViewed({
      id: plan.id, name: plan.name, countryName: plan.country.name, flag: plan.country.flag,
      price: plan.price, currency: plan.currency, data: plan.data, viewedAt: Date.now(),
    });
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <main id="main-content" className="flex-1 overflow-y-auto">
        <div className="p-6 md:p-8">
          <h1 className="font-display text-2xl font-bold mb-1">Buy a Plan</h1>
          <p className="text-muted-foreground text-sm mb-6">Choose the perfect data plan for your destination</p>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <div className="relative flex-1 min-w-56">
              ...
              <input type="search" value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search countries or plans..." aria-label="Search plans"
            ...
            <select value={filters.country || ''} onChange={e => handleFilter('country', e.target.value || undefined)}
            ...
            <select value={filters.sortBy || ''} onChange={e => handleFilter('sortBy', e.target.value as PlanFilters['sortBy'] || undefined)}
          ...
          {!plansLoading && plansData?.data.length === 0 && (
            <div className="py-24 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-5">Try different search terms or clear your filters.</p>
