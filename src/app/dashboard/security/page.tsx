...
export default function SecurityPage() {
  ...
  const pwField = (
    id: string,
    label: string,
    reg: 'currentPassword' | 'newPassword' | 'confirmPassword',
    show: boolean,
    toggle: () => void,
    error?: string,
  ) => (
    <div>
      ...
      <div className="relative">
        <input
          ...
          autoComplete={reg === 'currentPassword' ? 'current-password' : 'new-password'}
          aria-required="true"
          aria-invalid={!!error}
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <main id="main-content" className="max-w-3xl flex-1 overflow-y-auto p-6 md:p-8">
        <h1 className="mb-1 font-display text-2xl font-bold">Account Security</h1>
        <p className="mb-8 text-sm text-muted-foreground">
          Manage your password and account security settings
        </p>

        {/* Change Password */}
        <section aria-labelledby="change-pw-heading" className="mb-6 rounded-xl border bg-card p-6">
          <h2 id="change-pw-heading" className="mb-5 flex items-center gap-2 font-semibold">
            <KeyRound className="h-4 w-4 text-primary" aria-hidden="true" /> Change Password
          </h2>
          {success && (
            <div
              role="status"
              className="mb-4 rounded-lg border border-green-200 bg-green-100 p-3 text-sm text-green-700"
            >
        ...
        {/* Two-Factor Auth */}
        <section aria-labelledby="2fa-heading" className="mb-6 rounded-xl border bg-card p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 id="2fa-heading" className="flex items-center gap-2 font-semibold">
                <Shield className="h-4 w-4 text-primary" aria-hidden="true" /> Two-Factor
                Authentication
              </h2>
        ...
        {/* Active Sessions */}
        <section aria-labelledby="sessions-heading" className="mb-6 rounded-xl border bg-card p-6">
          <h2 id="sessions-heading" className="mb-4 font-semibold">
            Active Sessions
          </h2>
          {[
            {
