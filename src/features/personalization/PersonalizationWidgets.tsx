    ...
    (set, get) => ({
      ...
      addRecentlyViewed: (plan) =>
        set((s) => ({
          recentlyViewed: [
            plan,
            ...s.recentlyViewed.filter((p) => p.id !== plan.id),
          ].slice(0, 10),
        })),
...
export function SaveCountryButton({ countryCode, className }: { countryCode: string; className?: string }) {
  ...
  return (
    <button
      ...
      className={`flex h-9 w-9 items-center justify-center rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
        saved ? 'border-red-200 bg-red-50 text-red-500' : 'border-border hover:bg-muted text-muted-foreground'
      } ${className ?? ''}`}
    >
...
interface SavedCountriesWidgetProps { countries: Country[] }
  ...
  if (saved.length === 0) {
    return (
      <div className="rounded-xl border bg-card p-6 text-center">
        ...
        <p className="text-sm font-medium mb-1">No saved destinations yet</p>
        <p className="text-xs text-muted-foreground">Tap the heart icon on any country to save it here.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border bg-card p-5">
      <h2 className="font-semibold flex items-center gap-2 mb-4">
        <Heart className="h-4 w-4 text-red-500 fill-red-500" aria-hidden="true" /> Saved Destinations
      </h2>
      <ul className="grid grid-cols-2 sm:grid-cols-3 gap-2" role="list">
        {saved.map((c) => (
          <li key={c.code}>
            <div className="group relative flex items-center gap-2 rounded-lg border p-3 hover:bg-muted/50 transition-colors">
              <Link href={`${ROUTES.COUNTRIES}/${c.code.toLowerCase()}`} className="flex items-center gap-2 flex-1 min-w-0">
                <span className="text-xl flex-shrink-0" role="img" aria-label={c.name}>{c.flag}</span>
                <span className="text-sm font-medium truncate">{c.name}</span>
              </Link>
              <button
                onClick={() => toggleSaved(c.code)}
                className="flex-shrink-0 text-muted-foreground hover:text-destructive transition-colors opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded"
        ...
        {recentlyViewed.slice(0, 5).map((plan, i) => (
          ...
          >
            <Link
              ...
              className="flex items-center justify-between rounded-lg p-2.5 hover:bg-muted/50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="text-lg flex-shrink-0" role="img" aria-label={plan.countryName}>{plan.flag}</span>
                <div className="min-w-0">
                  <p className="text-sm font-medium truncate">{plan.name}</p>
                  <p className="text-xs text-muted-foreground">{formatDataGB(plan.data)} · {plan.countryName}</p>
                </div>
              </div>
              <span className="text-sm font-semibold flex-shrink-0">{formatCurrency(plan.price, plan.currency)}</span>
            </Link>
          </motion.li>
        ))}
      </ul>
...
interface RecommendedPlansWidgetProps { plans: Plan[] }
...
export function RecommendedPlansWidget({ plans }: RecommendedPlansWidgetProps) {
  ...
  const recommended = plans
    .filter((p) => interestedCountries.has(p.country.code) || interestedCountries.has(p.country.name))
    .slice(0, 4);
      ...
      <ul className="space-y-2" role="list">
        {fallback.map((plan, i) => (
          ...
          >
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="text-lg flex-shrink-0" role="img" aria-label={plan.country.name}>{plan.country.flag}</span>
                <div className="min-w-0">
                  <p className="text-sm font-medium truncate">{plan.name}</p>
                  <p className="text-xs text-muted-foreground">{formatDataGB(plan.data)} · {plan.country.name}</p>
                </div>
              </div>
              <span className="text-sm font-semibold flex-shrink-0">{formatCurrency(plan.price, plan.currency)}</span>
            </Link>
