  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <main id="main-content" className="max-w-5xl flex-1 overflow-y-auto p-6 md:p-8">
        <h1 className="mb-1 font-display text-2xl font-bold">Invoices</h1>
        <p className="mb-6 text-sm text-muted-foreground">
          {completed.length} invoice{completed.length !== 1 ? 's' : ''} available
        </p>
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center">
            ...
            <p className="mb-1 font-semibold">No invoices yet</p>
            <p className="text-sm text-muted-foreground">
              Completed orders will generate downloadable invoices here.
            </p>
          </div>
        ) : (
                  ...
                  <tr key={order.id} className="transition-colors hover:bg-muted/30">
                    ...
                    <td className="px-4 py-4">
                      <Button
                        ...
                        aria-label={`Download invoice ${order.id.slice(0, 8)}`}
                      >
                        PDF
                      </Button>
                    </td>
