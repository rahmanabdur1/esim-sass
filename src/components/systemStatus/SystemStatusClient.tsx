...
const MOCK_SERVICES: ServiceStatus[] = [
  ...
  },
  {
    ...
    status: 'operational',
    latency: 60,
    description: 'Ticket & chat support',
  },
];
...
export default function SystemStatusPage() {
  ...
  useEffect(() => {
    // Hydration mismatch রোধ করতে client-side-এ dynamic data set করা হয়েছে
  ...
  const handleRefresh = async () => {
    ...
    // Refresh-এর মাধ্যমে latency আপডেট করা হচ্ছে এবং setServices ব্যবহার করা হচ্ছে
    setServices((prevServices) =>
      prevServices.map((s) => ({
        ...s,
        latency: s.latency ? Math.floor(s.latency * (0.9 + Math.random() * 0.2)) : s.latency,
      })),
    );
      ...
      <main id="main-content" className="min-h-screen bg-background pt-16">
        ...
        <section className={`py-16 ${overallCfg.bg} border-b`} aria-labelledby="overall-status">
          <div className="container mx-auto px-4 text-center md:px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <OverallIcon
                className={`mx-auto mb-4 h-14 w-14 ${overallCfg.color}`}
                aria-hidden="true"
              />
              <h1
                id="overall-status"
                className={`mb-2 font-display text-3xl font-bold ${overallCfg.color}`}
              >
        ...
        <div className="container mx-auto max-w-4xl px-4 py-12 md:px-6">
          ...
          <section aria-labelledby="services-heading" className="mb-12">
            <h2 id="services-heading" className="mb-5 font-display text-xl font-bold">
              Services
            </h2>
              ...
              {services.map((svc, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-center justify-between border-b px-5 py-4 transition-colors last:border-0 hover:bg-muted/30"
                  >
          ...
          <section aria-labelledby="uptime-heading" className="mb-12">
            <div className="mb-5 flex items-center justify-between">
              <h2 id="uptime-heading" className="font-display text-xl font-bold">
                90-Day Uptime
              </h2>
            ...
            <div className="rounded-xl border bg-card p-5">
              <div
                className="flex min-h-[32px] items-end gap-0.5"
                role="img"
                ...
                {uptimeDays.map((day) => {
                  ...
                  return (
                    <div
                      ...
                      aria-hidden="true"
                      className={`h-8 flex-1 rounded-sm ${color} cursor-default opacity-80 transition-opacity hover:opacity-100`}
                    />
                  );
                })}
              </div>
          ...
          <section aria-labelledby="incidents-heading">
            <h2 id="incidents-heading" className="mb-5 font-display text-xl font-bold">
              Recent Incidents
            </h2>
