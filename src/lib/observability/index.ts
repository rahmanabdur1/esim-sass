...
export function reportWebVital(metric: VitalMetric): void {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    console.info(`[WebVitals] ${emoji} ${metric.name}: ${metric.value.toFixed(2)} (${rating})`);
  }
}
...
class JourneyTracker {
  ...
  planViewed(planId: string) {
    this.track('plan_viewed', { planId });
  }
  planSelected(planId: string) {
    this.track('plan_selected', { planId });
  }
  checkoutStarted(planId: string) {
    this.track('checkout_started', { planId });
  }
...
export function reportApiError(endpoint: string, statusCode: number, message: string): void {
