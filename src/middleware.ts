...
function checkRL(ip: string, type: LimitType) {
  ...
  if (e.count >= cfg.max) return { allowed: false, remaining: 0, resetAt: e.windowStart + cfg.window };
  e.count++;
...
function buildCSP(nonce: string, dev: boolean): string {
  ...
  const dirs: Record<string, string[]> = {
    'default-src':               [self],
    'script-src':                [self, `'nonce-${nonce}'`, 'https://js.stripe.com', ...(dev ? ["'unsafe-eval'"] : [])],
    'style-src':                 [self, "'unsafe-inline'", 'https://fonts.googleapis.com'],
    'font-src':                  [self, 'https://fonts.gstatic.com'],
    'img-src':                   [self, 'data:', 'blob:', 'https:'],
    'connect-src':               [self, process.env.NEXT_PUBLIC_API_URL ?? 'https://api.esimplatform.com', ...(dev ? ['ws://localhost:*'] : [])],
  ...
  return Object.entries(dirs).map(([k, v]) => v.length ? `${k} ${v.join(' ')}` : k).join('; ');
}
...
function log(level: LogLevel, msg: string, meta: Record<string, unknown>) {
  if (process.env.NODE_ENV === 'production') {
    console[level](JSON.stringify({ ts: new Date().toISOString(), level, msg, ...meta }));
  } else {
    const icon = { info: '✅', warn: '⚠️', error: '❌' }[level];
    console[level](`[MW] ${icon} ${msg}`, meta);
  }
}
...
function rateLimitResponse(resetAt: number): NextResponse {
  ...
  return new NextResponse(
    JSON.stringify({ success: false, code: 'RATE_LIMITED', message: 'Too many requests.', retryAfter: retry }),
...
export async function middleware(req: NextRequest): Promise<NextResponse> {
  ...
  // Check if route is explicitly public (static or dynamic prefix)
  ...
  if (MUTATING.has(method) && !isAuthPath) {
    ...
    const trusted = origin?.startsWith(appUrl) || referer?.startsWith(appUrl) || (!origin && !referer);
    if (!trusted) {
  ...
  // 11 ── Access log
  log('info', 'Request', {
    reqId, ip, method, pathname, loggedIn,
    ms: Date.now() - t0, rlRemaining: rl.remaining,
  });
