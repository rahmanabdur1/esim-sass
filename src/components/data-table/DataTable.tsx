...
function exportCSV<T>(data: T[], columns: ColumnDef<T>[], filename: string): void {
...
export function DataTable<T>({
  ...
  const SortIcon = ({ col }: { col: ColumnDef<T> }) => {
    ...
    if (!active)
      return <ChevronsUpDown className="h-3.5 w-3.5 text-muted-foreground/50" aria-hidden="true" />;
    if (sortDir === 'asc')
      return <ChevronUp className="h-3.5 w-3.5 text-primary" aria-hidden="true" />;
    ...
    <div className="space-y-3">
      ...
      {(searchable || exportable) && (
        <div className="flex flex-wrap items-center gap-3">
          {searchable && (
            <div className="relative min-w-52 flex-1" role="search">
              <Search
