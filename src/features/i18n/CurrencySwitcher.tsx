...
export const useCurrencyStore = create<CurrencyStore>()(
  persist(
    ...
    { name: 'currency-pref', storage: createJSONStorage(() => localStorage) }
  )
);
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1.5 rounded-md px-2 py-1.5 text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
        <DollarSign className="h-3.5 w-3.5" aria-hidden="true" />
        {!compact && <span className="hidden sm:inline">{cfg.flag} {currency}</span>}
        ...
        {open && (
          <>
            ...
            <motion.ul
              ...
              className="absolute right-0 top-full mt-1 z-50 w-52 rounded-xl border bg-popover shadow-lg py-1 overflow-hidden max-h-72 overflow-y-auto"
            >
              {(Object.keys(CURRENCIES) as CurrencyCode[]).map((code) => {
                ...
                return (
                  <li key={code} role="option" aria-selected={active}>
                    ...
                    >
                      <span className="text-base w-6 flex-shrink-0" aria-hidden="true">{c.flag}</span>
                      <span className="flex-1 text-left">
                        <span className="font-medium">{code}</span>
                        <span className="text-muted-foreground text-xs ml-2">{c.symbol}</span>
                      </span>
                      <span className="text-xs text-muted-foreground hidden sm:inline">{c.name.split(' ')[0]}</span>
                      {active && <Check className="h-3.5 w-3.5 text-primary flex-shrink-0" aria-hidden="true" />}
                    </button>
                  </li>
                );
              })}
