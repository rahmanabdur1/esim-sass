...
export function SessionTimeoutGuard({ children }: { children: React.ReactNode }) {
  const {
    lastActivity, isWarning, isTimedOut,
    warningDismissed, recordActivity, setWarning, setTimedOut,
  } = useSessionSecurityStore();
  ...
  useEffect(() => {
    timerRef.current = setInterval(() => {
      ...
      if (timeLeft <= 0 && !isTimedOut)       { setTimedOut(true); logout(); }
  ...
  const handleExtend = () => { recordActivity(); channelRef.current?.postMessage({ type: 'ACTIVITY' }); };
  const handleLogout = () => { channelRef.current?.postMessage({ type: 'LOGOUT' }); logout(); };
      ...
      <AnimatePresence>
        {isWarning && !warningDismissed && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 40 }}
            className="fixed bottom-6 right-6 z-50 w-80 rounded-2xl border bg-card shadow-2xl p-5"
          >
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h2 id="session-warn-title" className="font-semibold text-sm">Session Expiring</h2>
                <p id="session-warn-desc" className="text-xs text-muted-foreground mt-0.5">
                  Your session expires in <strong className="text-yellow-600">{formatSeconds(remaining)}</strong>. Continue?
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button size="sm" variant="gradient" className="flex-1" onClick={handleExtend}
